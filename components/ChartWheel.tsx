'use client'
import { Chart, SIGNS } from '@/lib/astro-data'

interface ChartWheelProps { chart: Chart; style?: 'renaissance' | 'minimal'; size?: number }

export default function ChartWheel({ chart, style = 'renaissance', size = 480 }: ChartWheelProps) {
  const scale = size / 480;
  const vbSize = 500;
  const cx = 250, cy = 250;
  const rOuter = 240 * scale;
  const rZodiac = 215 * scale;
  const rHouse = 165 * scale;
  const rInner = 100 * scale;
  const rPlanet = 140 * scale;

  // Convert longitude (0-360) to SVG angle. 0° Aries at 9 o'clock (left).
  // We rotate so ascendant is at 9 o'clock.
  const ascIdx = SIGNS.findIndex((s) => s.id === chart.ascendant.id);
  const ascLong = ascIdx * 30;
  const toAngle = (long: number) => {
    const a = (long - ascLong) * (Math.PI / 180);
    return -a + Math.PI; // 9 o'clock baseline, counterclockwise
  };
  const polar = (r: number, long: number): [number, number] => {
  ...
  const planetGroups = chart.planets.map((p, i) => {
    ...
    return (
      <g key={p.id}
         ...
         style={{ transformOrigin: `${cx}px ${cy}px`, animation: `wheelIn 0.7s cubic-bezier(0.34,1.56,0.64,1) ${0.2 + i * 0.07}s both` }}>
  ...
  return (
    <svg viewBox={`0 0 ${vbSize} ${vbSize}`} style={{ width: "100%", height: "100%" }}>
      {decor}
